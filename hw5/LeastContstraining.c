...
main()
{
      ...
      {    
          ...
          FILE *f_write;
          f_write=fopen("alg1_Schedule_file.txt","w");
...
void Schedule_Plan(bool *present_case,struct avaiable_schedule *schedule_array,int current_case_num)
{
      ...
      for (i=1;i<=selected_num;i++)//this is how to combine the min,min_ties,summary,and sections thers factors together,using number to value each factor
      {
            unique_array[i] = 1000000*min_array[i] + 10000*(case_num-min_ties_array[i]) + 10*summary_array[i] + (2-section_array[i]);
            unique_array[i] = unique_array[i]*10 + i;
      }
      for (i=1;i<=selected_num;i++)
      {
          int j;
          ...
          {
              int temp;
              if (unique_array[j]<=unique_array[j+1])
              {
                    temp=unique_array[j];
                    unique_array[j]=unique_array[j+1];
                    unique_array[j+1]=temp;
              }
          }
      }
      ...
      for (i=1;i<=selected_num;i++)//this loop is the branch ordering,important thing is to get which index in "selected_position[]" should be earlier than other
      {
          ...
          int a,b;
          a=unique_array[i]%10;//"a" gets the index in "selected_position[]"
          b=selected_position[a];//"b" gets the position in "schedule_array[]"
          ...
          label = copy_schedule_array[b].present_num;
          choosing_id = copy_schedule_array[b].attnory_id[label];//this is the selected attnory
          copy_schedule_array[b].present_num--;//have taken one out of this position
          
          bool temp_bitset[6];
          for (j=0;j<=5;j++)//assgin the bitset to a "temp_bitset",because using the temp_bitset is easy to calculate and do some operation
                temp_bitset[j] = copy_schedule_array[b].position_bit[j];
          ...
          if (fit_cases == case_num)
                stop_sign=1;
          result_array[fit_cases].attnory_id=choosing_id;
