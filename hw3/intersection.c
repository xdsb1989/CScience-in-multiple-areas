...
int superset_number=0;//for the whole file,how many customers have superset
int subset_number=0;//for the whole file,how many customers have subset
int disjoint_number=0;//for the whole file,how many customers have disjoin_number
float RMSE;
...
main()
{
     ...
     fclose(file);
     file=fopen("superset_file.txt","w");
     fclose(file);
     file=fopen("subset_file.txt","w");
     fclose(file);
     ...
     while (i<quiz_size)
     {
          current_cust=quiz_data[i];//get the customer ID which is going to be predicted
          current_movie=quiz_data[i+1];//get the movie ID which is going to be predicted
          FILE *file_predict;
          file_predict=fopen("prediction_file.txt","a");
          fprintf(file_predict,"%03d %05d %d ",current_cust,current_movie,quiz_data[i+2]);//out put in the prediction-file by using existed data
          fclose(file_predict);
          
          predict(original_data,off_set,current_cust,current_movie,quiz_data[i+2]);//the main method to predict rating
...
void predict(int *original_data,int *off_set,int current_cust,int current_movie,int real_rating)
{
     ...
     while (i<=max_ID)//
     {
           if (off_set[i]!=0 && i!=current_cust)//if off_set[i]!=0,that means here is a start
           {
                 ...
                 while (j<=section_stop)
                 {
                       ...
                       {
                              potential_set[k]=i;//i is the user's id,and put the i in the array
                              k++;
                              break;
                       }
                       j=j+2;
     ...
     int superset_array[user_number];//put the superset user id in superset_array
     int subset_array[user_number];//put the subset user id in subset_array
     int disjoint_array[user_number];//put the disjoint user id in disjoint_array
     
     int n=1;
     if (n<k)
     {
         int superset=0;//how many users are superset
         int subset=0;//how many users are subset
         int disjoint=0;//how many user are disjoint set
         ...
         while (n<k)//check all the potential id, and figure out which one is superset or subset or disjoint
         {
               ...
               int m;
               int temp_common=0;//how many common movies they have,if the common movies equals to the (label_stop-label_start+1)/2 number,it is a superset
               m=label_start;
               
               int j;
               j=section_start;
               while (m<=label_stop)//this loop is to choose all the movies from the comparing customer,the time complexity is O(n)
               {
                     while (original_data[m]>=original_data[j] && j<=section_stop)
                     {
               ...
               if (temp_common==(label_stop-label_start+1)/2)//this is a superset
               {
                   superset++;
                   superset_array[superset]=compare_id;
                   }
               else if (temp_common>0)//this is a subset
               {
                   subset++;
                   subset_array[subset]=compare_id;
                   }
               else if (temp_common==0)//this is a disjoint set
               {
                   disjoint++;
         ...
         if (superset>0)//the customer who has superset
         {
               ...
               int a;
               for (a=1;a<=superset;a++)////this loop is to choose each selected pair,superset elements
               {
                   ...
                   common_movie_number=(label_stop-label_start+1)/2;//using the start index and ending index to calculate the common movie number
                   float max_distance;
                   ...
                   {
                         while (original_data[m]>=original_data[i] && i<=compare_stop)//this loop is to choose each movie in the comparing customer
                         {
                   ...
                   similarity_array[a]=similarity;//store the similarity in the similarity array
                   total_similarity=total_similarity+similarity;//summary all the similarity together
                   
                   int n;
                   n=compare_start;
                   while (n<compare_stop)//this loop is to store all the score for current movie for each customer
                   {
         ...
         else if (superset==0 && subset>0)//the customer who has subset
         {
               ...
               int a;
               for (a=1;a<=subset;a++)//this loop is to choose each selected pair
               {
                   int compare_start;//the comparing customer's start index
                   int compare_stop;//the comparing customer's ending index
                   int compare_id;//the comparing customer's ID
                   ...
                   while (m<label_stop)//this loop is to choose each movie in the current customer,the time complexity is O(n)
                   {
                         while (original_data[m]>=original_data[i] && i<=compare_stop)//this loop is to choose each movie in the comparing customer
                         {
                               if (original_data[m]==original_data[i])
                               {
                                     ...
                                     float temp;
                                     temp=original_data[m+1]-original_data[i+1];
                                     temp=temp*temp;
                                     distance=distance+temp;//calculate the distance
                                     break;
                               }
                               i=i+2;
                         }
                   ...
                   distance=sqrt(distance/common_movie);//store all the different distance in the distance_array[]
                   total_commom_movie=total_commom_movie+common_movie;//store all the different common movies in the common_movie_array[]
