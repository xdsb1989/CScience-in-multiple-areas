...
void Build_Tree(struct decision_tree **,int *,int ,int *,int ,char *,int ,char *);
int LEVEL;

main()
{
      int type;
      printf("This is a limited Decision_tree algorithm\nsa-file at most 2 level toy-file at most 3 level.\nchoose sa-file press 1\nchoose toy-file press 2\ninput:");
      scanf("%d",&type);
      ...
      for (i=1;i<=different_words;i++)
            atributes_index[i]=i;      
      
      FILE *print_tree;
      print_tree=fopen("readable_tree.txt","w");
      fprintf(print_tree,"This is a limited tree in %d level\n1 means have the previous level's word,0 means don't have.\n",LEVEL);
      fclose(print_tree);
      char str1[]="|-";
      int level=1;
      Build_Tree(&root,example_index,email,atributes_index,different_words,str1,level,"");//while we build the tree,we also  print the tree
...
void Build_Tree(struct decision_tree **root,int *example_index,int emails,int *atributes_index,int current_words,char *str1,int level,char *tail)
{//example[] and atributes[] update level by level
       ...
       FILE *f;
       f=fopen("readable_tree.txt","a");
       
       if (level>LEVEL)//the tree at most have 4 levels, so if right now,the tree node is over 4 level,which means the node must be changed to a leaf
       {
       ...
       P_neg=1-P_pos;
       Info_before=-P_pos*(log(P_pos)/log(2))-P_neg*(log(P_neg)/log(2));
       ...
       //first jump to the branch which has the word
       Build_Tree(&((*root)->have_atribute),emails_have_word,number_have_word,atributes_index,current_words,str1,level,tail_have);
       //then jump to the branch which hasn't the word
       Build_Tree(&((*root)->without_atribute),emails_without_word,number_without_word,atributes_index,current_words,str2,level,tail_without);
}
